...
class Unsup3D():
    def __init__(self, cfgs):
        ...
        self.light_latent = False
        self.view_latent = True
        self.latent_option = 'add'
        ...
        self.netC = networks.ConfNet(cin=3, cout=2, nf=64, zdim=256)
    ...
    def forward(self, input):
        ...
        b, c, h, w = self.input_im.shape
         ...
         ## predict lighting
        canon_light, light_latent  = self.netL(self.input_im, latent=True)  # Bx4
        ...
        self.view, view_latent = self.netV(self.input_im, latent=True)
        ...
        latent_vector = view_latent
        ## predict canonical depth
        self.canon_depth_raw = self.netD(self.input_im, condition=latent_vector, option=self.latent_option).squeeze(1)  # BxHxW
        ...
        latent_vector = light_latent
        ## predict canonical albedo
        self.canon_albedo = self.netA(self.input_im, condition=latent_vector, option=self.latent_option)  # Bx3xHxW
        self.canon_albedo = torch.cat([self.canon_albedo, self.canon_albedo.flip(3)], 0)  # flip

        latent_vector = light_latent
        ## predict confidence map
        self.conf_sigma_l1, self.conf_sigma_percl = self.netC(self.input_im, condition=latent_vector, option=self.latent_option)  # Bx2xHxW
